...
for i=1:5
alpha=0.001*i;       % Hyperparameter 1 is varied for the sensitivity analysis
...
Jx(i,1)=trace((Xtr-(A(:,n+2*m+1:2*n+2*m)'*tanh(A(:,n+m+1:n+2*m)*(Xtr+(A(:,n+1:n+m)'*tanh(A(:,1:n)*Xtr))))))'*(Xtr-(A(:,n+2*m+1:2*n+2*m)'*tanh(A(:,n+m+1:n+2*m)*(Xtr+(A(:,n+1:n+m)'*tanh(A(:,1:n)*Xtr)))))));
Jy(i,1)=trace((Xtr+(A(:,n+1:n+m)'*tanh(A(:,1:n)*Xtr)))'*Q*(Xtr+(A(:,n+1:n+m)'*tanh(A(:,1:n)*Xtr))));
Jtheta(i,1)=trace(A'*A);
alpha_all(i,1)=alpha;
Vy_all(:,i)=Vytr;
i_all=linspace(1,10,10)';
end

%% Plotting results
figure(1)
    plot(alpha_all,log(Jx),'r-*','LineWidth',1.5)
    hold on
    plot(alpha_all,log(Jy),'b-*','LineWidth',2)
    hold on
    plot(alpha_all,log(Jtheta),'g-*','LineWidth',2)
    hold on
    xlabel('$\alpha$','Interpreter','latex','fontsize',15);ylabel('$log(J_{x}),log(J_{y}),log(J_{\theta})$','Interpreter','latex','fontsize',15);
    ...
    legend('$\alpha=0.001$','$\alpha=0.002$', '$\alpha=0.003$', '$\alpha=0.004$', '$\alpha=0.005$', 'Interpreter','latex','fontsize',15);
    grid on
    ax = gca;
    ax.GridAlpha = 0.3
    ax.GridLineStyle = ':'  