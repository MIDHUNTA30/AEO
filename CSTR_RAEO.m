...
h=11; 
V_threshold=99.9; % Variance threshold
m=n;
...
hold on
plot(Xtr(2,:),Xrtr(3,:),'b.','LineWidth',1)
hold on
ax = gca;
ax.GridAlpha = 0.5
ax.GridLineStyle = ':'
xlabel('$x_{2}$','Interpreter','latex','fontsize',20);ylabel('$x_{9}$','Interpreter','latex','fontsize',20);
legend('$x_{2}\hspace{.1cm} \mbox{vs} \hspace{.1cm} x_{9}$','$x_{2} \hspace{.1cm} \mbox{vs} \hspace{.1cm} x_{pr_9}(RAEO)$', 'Interpreter','latex','fontsize',15);
grid on
