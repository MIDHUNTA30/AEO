...
%% Simulating AEO for various q values
for i=1:length(Qq)
    q=Qq(i);    
    Q=diag([1,1*q^2]);    % Tuning parameter 5

    % Defining the loss function for AEO
    fun = @(A)alpha*trace((X-(A(:,n+2*m+1:2*n+2*m)'*tanh(A(:,n+m+1:n+2*m)*((A(:,n+1:n+m)'*tanh(A(:,1:n)*X))))))'*(X-(A(:,n+2*m+1:2*n+2*m)'*tanh(A(:,n+m+1:n+2*m)*((A(:,n+1:n+m)'*tanh(A(:,1:n)*X)))))))+beta*trace(((A(:,n+1:n+m)'*tanh(A(:,1:n)*X)))'*Q*((A(:,n+1:n+m)'*tanh(A(:,1:n)*X))))+gamma*trace(A'*A);
    % Training the autoencoder AEO
    ...
    Y= (A(:,n+1:n+m)'*tanh(A(:,1:n)*X)); 
    J1(:,i)=alpha*trace((X-(A(:,n+2*m+1:2*n+2*m)'*tanh(A(:,n+m+1:n+2*m)*((A(:,n+1:n+m)'*tanh(A(:,1:n)*X))))))'*(X-(A(:,n+2*m+1:2*n+2*m)'*tanh(A(:,n+m+1:n+2*m)*((A(:,n+1:n+m)'*tanh(A(:,1:n)*X)))))));
    J2(:,i)=beta*trace(((A(:,n+1:n+m)'*tanh(A(:,1:n)*X)))'*Q*((A(:,n+1:n+m)'*tanh(A(:,1:n)*X))));
    J3(:,i)=gamma*trace(A'*A);
    Vy=[var(Y(1,:));var(Y(2,:))];
    fl(:,i)=flag;
    Vyq(:,i)=Vy;
    J2a(:,i)=beta*(N-1)*(var(Y(1,:))+(q^2)*var(Y(2,:)));
    Xhat=(A(:,n+2*m+1:2*n+2*m)'*tanh(A(:,n+m+1:n+2*m)*((A(:,n+1:n+m)'*tanh(A(:,1:n)*X)))));
    ...
    fun0=@(Xr)A(:,n+p+1:n+m)'*tanh(A(:,1:p)*X(1:p,:)+A(:,p+1:n)*Xr);
...
figure(3)
    ...
    legend('$x_{1}\hspace{.1cm} \mbox{vs} \hspace{.1cm} x_{2}$','$x_{1} \hspace{.1cm} \mbox{vs} \hspace{.1cm} x_{pr_2}$', 'Interpreter','latex');
    grid on
    ax = gca;
    ax.GridAlpha = 1
    ax.GridLineStyle = ':' 
    subplot(2,1,2)
    plot(Qq,mseXr,'r.-','LineWidth',.7)
    xlabel('$q$','Interpreter','latex');ylabel('$MSE_{pr}$','Interpreter','latex');
    grid on
    ax = gca;
    ax.GridAlpha = 1
    ax.GridLineStyle = ':' 
