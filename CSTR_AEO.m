%% Description
% This code implements the Autoencoder with Ordered Variance (AEO) for 
...
options1 = optimoptions('fsolve','MaxIterations',1e7,'MaxFunctionEvaluations',1e7,'FunctionTolerance',1e-2,'OptimalityTolerance',1e-2','Algorithm','trust-region-dogleg')
...
h=11; 
V_threshold=99.9;  % Variance threshold percentage
m=n;
...
options = optimoptions('fminunc','MaxIterations',1e7,'MaxFunctionEvaluations',1e7,'OptimalityTolerance',1e-4','Algorithm','quasi-newton');
...
for s=1:1:n
   if (sum(Vytr(1:s))/sum(Vytr))*100 >= V_threshold
...
funtr=@(Xrtr)A(:,n+p+1:n+m)'*tanh(A(:,1:p)*Xtr(1:p,:)+A(:,p+1:n)*Xrtr);
...
Xrts0=Xts(p+1:n,:)+0.2*rand(n-p,Nts);
funts=@(Xrts)A(:,n+p+1:n+m)'*tanh(A(:,1:p)*Xts(1:p,:)+A(:,p+1:n)*Xrts);
